...
<script>
  ...
  import "./components/CodeMirrorCustom.css";
...
<main class="{$darkMode ? 'dark':''} flex flex-col h-screen text-slate-900 antialiased">  
  {#if uiState.modalAnalyze === true}
    <div class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white w-6/12 rounded overflow-hidden shadow" style="z-index:90;" id="modal-sample">
      <div class="flex flex-col" style="max-height:calc(100vh - 40px);">
        ...
        <div class="px-3 py-2 border-0 border-t border-slate-300 dark:border-0 dark:bg-slate-900 z-10">
          <div class="flex items-center">
            ...
            <button on:click={() => (uiState.modalActive = false, uiState.modalHeaders = false)} class="ml-auto btn-primary">Save Header</button>
          </div>
        </div>  
      </div>
    </div>
  ...
  {#if uiState.modalHeaders === true}
    <div class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white w-6/12 rounded overflow-hidden shadow" style="z-index:90;" id="modal-sample">
      <div class="flex flex-col" style="max-height:calc(100vh - 40px);">
        <div class="px-3 py-2 border-0 border-b border-slate-300 dark:border-0 dark:bg-slate-900 z-10">
          <p class="font-semibold text-base dark:text-slate-50">Modify Headers</p>
          <p class="text-slate-600 text-sm dark:text-slate-400">Set the headers which are sent along with your GraphQL request.</p>
          <p class="text-slate-600 text-sm dark:text-slate-400">As an administrator, you're able to set <span class="font-mono px-1 py-0.5 whitespace-nowrap rounded bg-slate-100 mx-1">x-hasura...</span> headers like <span class="font-mono px-1 py-0.5 whitespace-nowrap rounded bg-slate-100 mx-1">x-hasura-role</span> and <span class="font-mono px-1 py-0.5 rounded bg-slate-100 mx-1">x-hasura-user</span> to emulate the effects of a JWT token's permissions.</p>
        </div>
        <div class="flex flex-grow bg-slate-50 overflow-auto dark:bg-slate-800">
  ...
  <div id="pageHead" class="relative flex items-center px-3 py-2 shadow bg-white dark:bg-slate-900 z-20">
    ...
    <div 
      ...
      class="flex items-center cursor-pointer text-sm text-slate-600 dark:text-slate-400 px-2 py-1 mr-3 w-max rounded hover:bg-slate-100 dark:hover:bg-slate-700">
  ...
  <div class="{$darkMode ? 'bg-grid-dark':'bg-grid'} flex-grow p-3 overflow-none">
    {#await graphqlSchema}
      <p class="text-center">Loading Schema...</p>
    {:then}
      <ReactAdapter
        el={TykGraphiQL}
        fetcher={fetcher}
  ...
  <div id="pageFoot" class="relative flex items-center px-3 py-2 shadow bg-white dark:bg-slate-900 z-20">
    <span class="font-semibold dark:text-slate-50 whitespace-nowrap mr-1">Last Query</span>
    <div class="flex items-center cursor-pointer text-sm text-slate-600 dark:text-slate-400 px-2 py-1 mr-3 w-max rounded hover:bg-slate-100 dark:hover:bg-slate-700">
      <span class="font-mono mr-1 line-clamp-1">{query}</span>
      <Icon size="14" className="text-slate-400 fill-current ml-1" src={IoFlask} />
    </div>
    <div use:clickOutside on:outside={() => (uiState.createFromQueryDropState = false)} class="relative ml-auto">
